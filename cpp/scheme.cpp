...
struct ExpSequence : public Expression {
  ...
  virtual Expression* eval(Environment* e) {
    assert(es.size() > 0);
    Expression* res = 0;
    for (int i = 0; i < es.size(); i++) res = es[i]->eval(e);
    return res;
  }
  virtual string toString() {
    string res = "";
    for (int i = 0; i < es.size(); i++) res += es[i]->toString();
    return res;
  }
};
...
struct Application : Expression {
  ...
  virtual string toString() {
    ...
    for (int i = 0; i < args.size(); i++) s += " " + args[i]->toString();
    s += ")";
    return s;
  }
  virtual Expression* eval(Environment* e) {
    ...
    if (ISA(opVal, Label*)) {
      ...
      if (e->has(s))
        ...
        return new IntValue(((IntValue*)vals[0])->v + ((IntValue*)vals[1])->v);
      else if (s == "-")
        return new IntValue(((IntValue*)vals[0])->v - ((IntValue*)vals[1])->v);
      else if (s == "*")
        return new IntValue(((IntValue*)vals[0])->v * ((IntValue*)vals[1])->v);
    ...
    } else if (ISA(opVal, Lambda*))
      f = (Lambda*)opVal;
    Environment* e2 = new Environment();
    e2->parent = f->e;
    for (int i = 0; i < f->args.size(); i++) e2->m[f->args[i]] = vals[i];
...
struct If : Expression {
  ...
  virtual string toString() {
    return "(if " + condition->toString() + " " + thenPart->toString() + " " +
           elsePart->toString() + ")";
  }
  virtual Expression* eval(Environment* e) {
